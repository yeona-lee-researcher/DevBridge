...
function SkillsTab() {
  ...
  const initSkills = (clientProfileDetail?.skills || []).length > 0
    ? clientProfileDetail.skills.map(s => ({ ...s, mode: "saved" }))
...
function CareerTab() {
  ...
  const initCareers = (clientProfileDetail?.careers || []).length > 0
    ? clientProfileDetail.careers.map(c => ({ ...c, mode: "saved", company: c.companyName || c.company || "", mainTech: c.jobTitle || c.mainTech || "" }))
...
function EducationTab() {
  const { clientProfileDetail, updateClientProfileDetail } = useStore();
  const [savedEntries, setSavedEntries] = useState(clientProfileDetail?.educations || []);
...
function CertificatesTab() {
  ...
  const initCerts = (clientProfileDetail?.certifications || []).length > 0
    ? clientProfileDetail.certifications.map(c => ({ id: c.id, mode: "saved", name: c.certName || c.name, org: c.issuer || c.org, date: c.acquiredDate || c.date }))
...
function AwardsTab() {
  ...
  const initAwards = (clientProfileDetail?.awards || []).length > 0
    ? clientProfileDetail.awards.map(a => ({ id: a.id, mode: "saved", name: a.awardName || a.name, org: a.awarding || a.org, date: a.awardDate || a.date, desc: a.description || a.desc || "" }))
